...
void script_out(ofstream &fout, double** a, double* b, int n, int m);
...
int main()
{
  ...
  script_init(out);
  script_out(out, results, joules_results, n_vel, n_time);
  script_finish(out);
  out.close();
  delete[] results;
...
void script_out(ofstream &fout, double** a, double* b, int n, int m)
{
  ...
  for(int i=0; i<n; i++)
  {
    for(int j=0; j<m; j++)
    {
      ...
      if(j==0)
      {
        fout << "title('Initial Velocity "<< 25+5*i <<"MPH')" << endl;
        fout << "xlabel('Initial Deceleration - MPS')" << endl;
        fout << "ylabel('Joules Generated')" << endl;
        fout << "hold on" << endl;
      }
      if(j==m-1)
      {
